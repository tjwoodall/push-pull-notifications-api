...
class RetryPushNotificationsJobSpec extends AsyncHmrcSpec with GuiceOneAppPerSuite with FixedClock {
  ...
  class Setup(val batch: Int = 5) extends NotificationsRepositoryMockModule with NotificationPushServiceMockModule with MongoLockRepositoryMockModule with TestData {
    ...
    val jobConfig: RetryPushNotificationsJobConfig = RetryPushNotificationsJobConfig(
    ...
    def runBatchTest(numberBad: Int, numberGood: Int)(implicit ec: ExecutionContext) = {
      ...
      val result: String = await(underTest.execute)
  ...
  "RetryPushNotificationsJob" should {
    ...
    "handle error when something fails" in new Setup {
      ...
      result shouldBe "The execution of scheduled job RetryPushNotificationsJob failed with error 'Failed'. " +
        "The next execution of the job will do retry."
    }

    "attempt to send all even if 1st fails with one batch worth of requests" in new Setup(5) {
      runBatchTest(numberBad = 2, numberGood = 3) shouldBe "RetryPushNotificationsJob Job ran successfully."
    }

    "attempt to send all even if 1st fails with more than one batch worth of requests" in new Setup(5) {
      runBatchTest(numberBad = 1, numberGood = 15) shouldBe "RetryPushNotificationsJob Job ran successfully."
    }
  }
}
